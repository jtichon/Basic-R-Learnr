...
Create a vector <span class="tt">x</span> that contains the integers from 1 to 6. Use R to calculate the values that are one less than three times the values in <span class="tt">x</span>. Note you will need to combine two arithmetic operations in the correct order.
...
to create your dataframe. If you are unfamiliar with making csv files, you can also make a spreadsheet in a program such as Excel or Sheets and ask it to save your as a csv file.
...
If we wanted information on Genevieve who appears in the fourth row (we exclude the column names form the rows), we can go
...
Andrea is dungeon mastering a role playing campaign with her friends. She has tracked information on all of the players characters: the **Character** name, their **Class**, the amount of **Gold**, and their player **Level**. All of this data has been entered into a dataframe in R. Here is a printout of her dataframe named **rpg**:
